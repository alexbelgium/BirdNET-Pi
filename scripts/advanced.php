...
if(isset($_GET['submit'])) {
  ...
  if(isset($_GET["birdnetpi_url"])) {
    ...
    $birdnetpi_url = rtrim($birdnetpi_url, '/'); // remove trailing slash
...
<head>
  ...
  <link rel="stylesheet" href="style.css?v=3.28.25">
  ...
  <style>
    ...
    #speciesCountModal div {
      display: flex;
      align-items: center;
      margin-bottom: 0.5em;
    }
    #speciesCountModal button {
      height: 32px;
      margin-left: 5px;
      padding: 0 10px;
...
?>
  ...
  <form id="advancedform" action="" method="GET">
    ...
    <table class="settingstable"><tr><td>
      ...
      <label for="purge">
        <input name="full_disk" type="radio" id="purge" value="purge" <?php if (strcmp($newconfig['FULL_DISK'], "purge") == 0) { echo "checked"; }?>>Purge
      </label>
      <label for="keep">
        <input name="full_disk" type="radio" id="keep" value="keep" <?php if (strcmp($newconfig['FULL_DISK'], "keep") == 0) { echo "checked"; }?>>Keep
      </label>
      ...
      <p>Defines how full the disk should be before the purge operations occur.<br>Note: This variable is still active if Keep is set. This means that the services will be stopped at the purge threshold.</p><br>
    ...
    <!-- Disk Usage Summary Modal Trigger and Modal -->
    <button type="button" class="testbtn" id="runSpeciesCountBtn">[Click here for disk usage summary]</button>
    <dialog id="speciesCountModal">
      <div>
        <h3>Disk Usage Summary</h3>
      </div>
      <pre id="speciesCountOutput">Loading...</pre>
      <button type="button" id="closeSpeciesCountModal">Close</button>
    </dialog>
    
    <!-- (The rest of your Advanced Settings form below remains unchanged) -->
    <table class="settingstable"><tr><td>
...
<?php
  ...
  foreach ($rtsp_streams as $stream_idx => $stream_url) {
    if ($stream_idx === 0) {
      echo "<input id='rtsp_stream_url_0' name='rtsp_stream_0' type='url' size='60' value='$stream_url'><br>";
    } else {
      echo "<input id='rtsp_stream_url_$stream_idx' name='rtsp_stream_$stream_idx' type='url' size='60' value='$stream_url'><br>";
    }
  }
?>
      ...
      <p>If you place an RTSP stream URL here, BirdNET-Pi will use that as its audio source.<br>Multiple streams are allowed but may have an impact on rPi performance.<br>Analyze ffmpeg CPU/Memory usage with <b>top</b> or <b>htop</b> if necessary.<br>To remove all and use the soundcard again, just delete the RTSP entries and click Save at the bottom.</p>
    </td></tr></table><br>
    ...
    <table class="settingstable"><tr><td>
      ...
      <input style="width:40ch" name="caddy_pwd" id="caddy_pwd" type="password" pattern="[A-Za-z0-9]+" title="Password must be alphanumeric (A-Z, 0-9)" value="<?php print($newconfig['CADDY_PWD']);?>" />
      <span id="showpassword" onmouseover="document.getElementById('caddy_pwd').type='text';" onmouseout="document.getElementById('caddy_pwd').type='password';">show</span><br>
    </td></tr></table><br>
    ...
    <table class="settingstable"><tr><td>
      ...
      <p>This allows you to quiet the display of how many commits your installation is behind relative to the Github repo. This number appears next to "Tools" when you're 50 or more commits behind.</p><br>
      ...
      <p>This allows you to remove the axes and labels of the spectrograms generated by Sox for each detection for a cleaner appearance.</p><br>
    ...
    <table class="settingstable"><tr><td>
      ...
      <p>This visually marks species that were manually confirmed as present. A question mark appears next to species names in the Recordings page; clicking it changes the icon to a checkmark and adds the species to confirmed_species_list.txt.</p>
    </td></tr></table><br>
    ...
    <table class="settingstable"><tr><td>
      ...
      <p>These allow you to show a custom image on the Overview page of your BirdNET-Pi. For example, a dynamically updating picture of your garden.</p>
    </td></tr></table><br>
    ...
    <table class="settingstable"><tr><td>
      ...
      <p>Birdsongs Frequency shifting configuration:<br>
        This can be useful for hearing-impaired users. Note: audio files will only be pitch shifted when the "FREQ SHIFT" button is manually clicked for a detection on the Recordings page.<br>
        The frequency shifting can also be activated for the realtime audio livestream (in the SPECTROGRAM tab). Once activated, it will be available for Live Audio as well. Livestream uses ffmpeg for streaming, so the pitch shifter will use that tool too. If you choose sox for recorded audio files, configure both sox and ffmpeg parameters accordingly.
      </p>
...
?>
      ...
      <p style="margin-left: 40px">
        <label for="freqshift_reconnect_delay">Livestream reconnection delay (ms): </label>
      ...
      <p style="margin-left: 40px">
        Using sox:<br>
        e.g. shiftPitch=-1200 shifts one octave down (value in 100ths of a semitone).<br>
      ...
      <tr>
        <td>
          ...
          <div class="callout callout-warning">
            <b>Note:</b> It is recommended that the Log Level be set to <b>Error</b> on production systems to keep output manageable by reporting only errors.<br>
      ...
      <tr>
        <td>Birdnet Recording:
          <select class="testbtn" id="LogLevel_BirdnetRecordingService" name="LogLevel_BirdnetRecordingService">
            <option value="error" <?php echo $newconfig['LogLevel_BirdnetRecordingService'] == "error" || !array_key_exists('LogLevel_BirdnetRecordingService', $newconfig) ? "selected=''" : "" ?>>Errors Only</option>
            <option value="warning" <?php echo $newconfig['LogLevel_BirdnetRecordingService'] == "warning" ? "selected=''" : "" ?>>Warning</option>
            <option value="info" <?php echo $newconfig['LogLevel_BirdnetRecordingService'] == "info" ? "selected=''" : "" ?>>Info</option>
            <option value="debug" <?php echo $newconfig['LogLevel_BirdnetRecordingService'] == "debug" ? "selected=''" : "" ?>>Debug</option>
          </select>
        </td>
      </tr>
      <tr>
        <td>Live Audio Stream:
          <select class="testbtn" id="LogLevel_LiveAudioStreamService" name="LogLevel_LiveAudioStreamService">
            <option value="error" <?php echo $newconfig['LogLevel_LiveAudioStreamService'] == "error" || !array_key_exists('LogLevel_LiveAudioStreamService', $newconfig) ? "selected=''" : "" ?>>Errors Only</option>
            <option value="warning" <?php echo $newconfig['LogLevel_LiveAudioStreamService'] == "warning" ? "selected=''" : "" ?>>Warning</option>
            <option value="info" <?php echo $newconfig['LogLevel_LiveAudioStreamService'] == "info" ? "selected=''" : "" ?>>Info</option>
            <option value="debug" <?php echo $newconfig['LogLevel_LiveAudioStreamService'] == "debug" ? "selected=''" : "" ?>>Debug</option>
          </select>
        </td>
      </tr>
      <tr>
        <td>Spectrogram Service:
          <select class="testbtn" id="LogLevel_SpectrogramViewerService" name="LogLevel_SpectrogramViewerService">
            <option value="error" <?php echo $newconfig['LogLevel_SpectrogramViewerService'] == "error" || !array_key_exists('LogLevel_SpectrogramViewerService', $newconfig) ? "selected=''" : "" ?>>Errors Only</option>
            <option value="warning" <?php echo $newconfig['LogLevel_SpectrogramViewerService'] == "warning" ? "selected=''" : "" ?>>Warning</option>
            <option value="info" <?php echo $newconfig['LogLevel_SpectrogramViewerService'] == "info" ? "selected=''" : "" ?>>Info</option>
            <option value="debug" <?php echo $newconfig['LogLevel_SpectrogramViewerService'] == "debug" ? "selected=''" : "" ?>>Debug</option>
          </select>
        </td>
      </tr>
      <tr>
        <td>
          <small>
            <b>'error'</b> - Show all errors, including recoverable ones. <b>This is the default value.</b><br>
            <b>'warning'</b> - Show warnings and errors.<br>
            <b>'info'</b> - Show informative messages in addition to warnings and errors (more output).<br>
            <b>'debug'</b> - Show everything, including debugging information (lots of output).<br>
          </small>
        </td>
      </tr>
    </table>
...
<!-- Include dialog polyfill JS -->
<script src="static/dialog-polyfill.js"></script>
<script>
  // Register the modal with the polyfill
  const speciesCountModal = document.getElementById('speciesCountModal');
  dialogPolyfill.registerDialog(speciesCountModal);
  const speciesCountOutput = document.getElementById('speciesCountOutput');
  const runSpeciesCountBtn = document.getElementById('runSpeciesCountBtn');
  const closeSpeciesCountModal = document.getElementById('closeSpeciesCountModal');

  runSpeciesCountBtn.addEventListener('click', () => {
    speciesCountOutput.textContent = "Loading...";
    speciesCountModal.showModal();

    // AJAX call to disk_species_count.php
    const xhr = new XMLHttpRequest();
    xhr.onreadystatechange = function() {
      if (xhr.readyState === XMLHttpRequest.DONE) {
        speciesCountOutput.textContent = xhr.responseText;
      }
    };
    xhr.open('GET', 'disk_species_count.php', true);
    xhr.send();
  });

  // Add event listener to the Close button within the modal
  document.getElementById('closeSpeciesCountModal').addEventListener('click', () => {
    speciesCountModal.close();
  });
</script>
</body>
</html>
