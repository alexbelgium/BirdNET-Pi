...
while($results=$result3->fetchArray(SQLITE3_ASSOC)){
  ...
  $header = "<div style='text-align:center'><h3>$species</h3><table style='margin:0 auto'><tr><td class='relative'>";
  $header .= '<a target="_blank" href="index.php?filename=' . $results['File_Name'] . '"><img title="Open in new tab" class="copyimage" width="25" src="images/copy.png"></a><i>' . $sciname . '</i>';
  $header .= "<a href='$url' target='_blank'><img style='width: unset !important; display: inline; height: 1em; cursor: pointer;' title='$url_title' src='images/info.png' width='20'></a>";
  $header .= "<a href='https://wikipedia.org/wiki/$sciname' target='_blank'><img style='width: unset !important; display: inline; height: 1em; cursor: pointer;' title='Wikipedia' src='images/wiki.png' width='20'></a>";
  $header .= "<img style='height: 1em;cursor:pointer;float:unset;display:inline' title='View species stats' onclick=\"generateMiniGraph(this, '" . $comnamegraph . "', 180)\" width='25' src='images/chart.svg'><br>";
  $header .= "Occurrences: $count<br>Max Confidence: $maxconf<br>Best Recording: $date $time<br><br>";
  $header .= "<div class='custom-audio-player' data-audio-src='$filename' data-image-src='$filename.png'></div>";
  $header .= "</td></tr></table></div>";
  echo str_pad($header."<p>Loading Images from ".ucfirst($image_provider_name)."</p>", '6096');
