...
def sendAppriseNotifications(species, confidence, confidencepct, path,
    ...
    if APPRISE_ONLY_NOTIFY_SPECIES_NAMES is not None and APPRISE_ONLY_NOTIFY_SPECIES_NAMES.strip() != "":
        if any(bird.lower().replace(" ", "") in comName.lower().replace(" ", "")
               for bird in APPRISE_ONLY_NOTIFY_SPECIES_NAMES.split(",")):
            return
    ...
    if APPRISE_ONLY_NOTIFY_SPECIES_NAMES_2 is not None and APPRISE_ONLY_NOTIFY_SPECIES_NAMES_2.strip() != "":
        if not any(bird.lower().replace(" ", "") in comName.lower().replace(" ", "")
                   for bird in APPRISE_ONLY_NOTIFY_SPECIES_NAMES_2.split(",")):
            return

    APPRISE_MINIMUM_SECONDS_BETWEEN_NOTIFICATIONS_PER_SPECIES = settings_dict.get(
    ...
    if settings_dict.get('APPRISE_NOTIFY_NEW_SPECIES_EACH_DAY') == "1":
        try:
            ...
            numberDetections = detections[0] if len(detections) else 0
