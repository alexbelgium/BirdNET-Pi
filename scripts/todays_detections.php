...
$stmt = $db->prepare("SELECT Date, COUNT(*) AS Detections FROM detections WHERE REPLACE(Com_Name, '''', '') = REPLACE(:com_name, '''', '') AND Date BETWEEN DATE('now', '-$days days') AND DATE('now') GROUP BY Date");
