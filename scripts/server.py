...
def run_bats_analysis(file, host="127.0.0.1", port=7667):
    ...
    for segment, entries in data.get("results", {}).items():
        ...
        for species, score_str in entries:
            ...
            if (
                score < min_conf
                or (include and species not in include)
                or (exclude and species in exclude)
                or (PREDICTED_SPECIES_LIST and species not in PREDICTED_SPECIES_LIST)
            ):
                continue
            if score > max_score:
                max_score = score
        ...
        if max_species not in ["", "noise", "electrical", "mechanical"]:
            detections.append(
                Detection(file.file_date, start, stop, max_species, max_score)
            )
            log.info(
                "Detected %s with confidence %s at start %s and stop %s ",
                max_species,
                max_score,
                start,
                stop,
            ...
            log.info(
                "No species detected with required confidence %s between %s and %s",
                min_conf,
                start,
                stop,
            )
